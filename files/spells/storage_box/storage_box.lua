...
local trigger = (inputs.frameinteract == GameGetFrameNum() or inputs.framefire == GameGetFrameNum()) and (frame > openframe + 1)
...
if is_destroying and destroy then
	...
	for i = 1, #types do
		local num = destroy[types[i].id] or 0
		count = count + (num > 0 and 1 or 0)
	end
	local divider = 20 * anim
	local spacing = 14 * anim
	local dx = cx + (divider * -0.5 * count) + divider * 0.5
