...
function GetLocation(file_name)
    ...
    for i = 1, #v do
        if ComponentGetValue2(v[i], "name") == file_name then
            -- return exits the function here so the component below doesn't get added
...
function SetLocation(file_name, thing)
    local world = EntityGetWithName("ns_world_handler")
    local v = EntityGetComponent(world, "VariableStorageComponent") or {}
    for i = 1, #v do
        if ComponentGetValue2(v[i], "name") == file_name then
            ComponentSetValue2(v[i], "value_string", smallfolk.dumps(thing))
        end
    end
end
...
function Load_location(name, x, y, freeze_frames, folder)
    ...
    GlobalsSetValue("NS_LOCATION_FILEPATH", folder)
    if location and world and player then
        ...
        if vs then
            ...
            ComponentSetValue2(vs, "value_float", freeze_frames or 2)
        end
