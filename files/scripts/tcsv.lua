...
---@return string
---@nodiscard
local function serialize_csv_field(value)
    -- CSV fields containing a comma, quote, or newline must be quoted.
    -- Quotes inside a quoted field are escaped by doubling them.
    if value:find('[,"\r\n]') then
        value = value:gsub('"', '""')
        return '"' .. value .. '"'
    end
...
---@return string
---@nodiscard
function tcsv.serialize(csv)
    local lines = {}

    local header = {""}

    for _, lang in ipairs(csv.langs) do
        header[#header+1] = serialize_csv_field(lang)
    end
    ...
    for _, row in ipairs(csv.rows) do
        ...
        for i, value in ipairs(row) do
            ...
            fields[#fields+1] = serialize_csv_field(value)
        end
